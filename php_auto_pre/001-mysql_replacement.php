...
foreach ($xxx_directories_need_mysql as $xxx_directory_need_mysql) { /* @phpstan-ignore-line */
	...
	if (strpos(realpath($_SERVER['SCRIPT_FILENAME']??''), realpath($xxx_directory_need_mysql)) === 0) {
		$xxx_go = !$xxx_negate;
	}
	if (strpos(realpath(rtrim($_SERVER['PWD']??'',DIRECTORY_SEPARATOR)), realpath(rtrim($xxx_directory_need_mysql,DIRECTORY_SEPARATOR))) === 0) {
		$xxx_go = !$xxx_negate;
	}
	unset($xxx_negate);
}
