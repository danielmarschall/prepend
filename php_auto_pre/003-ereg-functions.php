...
foreach ($xxx_directories_need_ereg as $xxx_directory_need_ereg) { /* @phpstan-ignore-line */
	...
	if (strpos(realpath($_SERVER['SCRIPT_FILENAME']??''), realpath($xxx_directory_need_ereg)) === 0) {
		$xxx_go = !$xxx_negate;
	}
	if (strpos(realpath(rtrim($_SERVER['PWD']??'',DIRECTORY_SEPARATOR)), realpath(rtrim($xxx_directory_need_ereg,DIRECTORY_SEPARATOR))) === 0) {
		$xxx_go = !$xxx_negate;
	}
	unset($xxx_negate);
}
